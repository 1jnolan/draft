...
st_autorefresh(interval=30000, key="fpl_refresh_draft_choices")
...
def fetch_bootstrap_data():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for el in elements:
                player_map[el["id"]] = {
                    "Player Name": f"{el.get('first_name', '')} {el.get('second_name', '')}".strip(),
                    "Position": positions.get(el.get("element_type"), "N/A"),
                    "Club": teams.get(el.get("team"), "N/A"),
                }
            return player_map
    except Exception:
        pass
...
if league_data and draft_data and player_map:
    ...
    if choices:
        ...
        for choice in choices:
            ...
            drafted_list.append({
                "Pick #": choice.get("index", 0),
                "Round": choice.get("round", 0),
        ...
        m3.metric("Draft Rounds", int(df_draft["Round"].max()) if not df_draft.empty else 0)

        st.divider()

        # Filters
        c1, c2, c3 = st.columns(3)
        selected_manager = c1.selectbox(
            "Filter by Manager:",
            ["All Managers"] + sorted(list(df_draft["Drafted By"].unique())),
        )
        selected_position = c2.selectbox(
            "Filter by Position:",
            ["All Positions"] + sorted(list(df_draft["Position"].unique())),
        )
        selected_club = c3.selectbox(
            "Filter by Club:",
            ["All Clubs"] + sorted(list(df_draft["Club"].unique())),
        )
        ...
        st.info("⌛ **Draft Pending:** No choices recorded yet for League 858.")
